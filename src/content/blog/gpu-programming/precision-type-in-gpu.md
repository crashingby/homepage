...
位布局不是简单的 `sign | exponent | mantissa`。一套浮点格式还必须规定：哪些 bit pattern 表示有限数，哪些保留给 `±0`、subnormal（次正规数）、`±Inf` 和 NaN；否则“最大值”“最小值”都没有唯一答案。

以下用 `s1 e<k> m<m>` 描述一个格式：`s` 为 1 个符号位，`e` 为 $k$ 个指数位，`m` 为 $m$ 个 fraction（小数字段）位。令 $E$ 是指数位解释出的无符号整数，$F$ 是 fraction 位解释出的无符号整数。常见 IEEE 风格的指数偏置为：

$$
bias = 2^{k-1} - 1
$$

例如 FP32 是 `s1 e8 m23`，所以 `bias = 127`；FP16 是 `s1 e5 m10`，所以 `bias = 15`。`mantissa` 在日常表述中常指整个有效数，但 bit 字段本身更准确叫 `fraction`；下文统一用 **有效数**（significand）指参与乘法的 $1.f$ 或 $0.f$。

### IEEE 风格格式：四类 bit pattern

对于保留全 1 指数给 `Inf` / NaN 的 IEEE 754 二进制格式，所有 bit pattern 可按下表解码。FP16、FP32、FP64 和常见的 BF16 都遵循这一核心规则。

| 指数 $E$ | fraction $F$ | 表示的值 | 含义 |
| --- | --- | --- | --- |
| $1 \le E \le 2^k - 2$ | 任意 | $(-1)^s \times (1 + F\,2^{-m}) \times 2^{E-bias}$ | **正常数**（normal）。有效数有隐式前导 `1`。 |
| $0$ | $0$ | $(-1)^s \times 0$ | **`+0` / `-0`**。两者比较相等，但在 `1 / ±0`、符号传播等边界计算中可观察到符号。 |
| $0$ | $1 \le F \le 2^m - 1$ | $(-1)^s \times (F\,2^{-m}) \times 2^{1-bias}$ | **subnormal**。没有隐式前导 `1`。 |
| $2^k - 1$ | $0$ | $(-1)^s \times \infty$ | **`+Inf` / `-Inf`**。有限数 overflow 的 IEEE 默认结果。 |
| $2^k - 1$ | $F \ne 0$ | NaN | **NaN**。无效运算（如 `0/0`、`Inf - Inf`）等可产生它；fraction 通常携带 payload。 |

正常数的 `1` 不存入 bit，因此 $m$ 个 fraction 位能给出 $m+1$ bit 的有效精度。以 FP32 的 $1.01_2 \times 2^3$ 为例，存储的只是 `s=0`、`E=127+3` 和 fraction `010...0`；前导 `1` 由“$E$ 既非全 0 也非全 1”这条规则补回。
...
$$
x_{\min,\mathrm{normal}} = 2^{1-bias}
$$

若没有 subnormal，任何小于它的非零结果都只能突然变为 0，造成明显的 underflow 断层。subnormal 改用 $0.f$，固定实际指数为 $1-bias$，让可表示值以固定步长一路延伸到 0：

$$
x_{\min,\mathrm{subnormal}} = 2^{1-bias-m}
$$

$$
x_{\max,\mathrm{subnormal}} = (1-2^{-m}) \times 2^{1-bias}
$$

因此最大 subnormal 与最小 normal 只差一个 subnormal 步长 $2^{1-bias-m}$，数轴在 0 附近是连续衔接的；代价是 subnormal 区没有隐式 `1`，有效位数会随着数值接近 0 而减少。

- FP16：最小 normal 是 $2^{-14}$，最小 subnormal 是 $2^{-24}$。
- FP32：最小 normal 是 $2^{-126}$，最小 subnormal 是 $2^{-149}$。
- 这个机制只保证**逐渐下溢**，不保证相对精度不变。GPU 的某些硬件路径或编译选项也可能将 subnormal flush-to-zero（FTZ）；性能敏感代码不能假设所有路径都保留它们。

### 有限值的最大、最小与相邻间隔

“最小值”必须先说清含义：数轴上的最小有限值是负的最大有限值；通常说的“最小正数”又要区分 normal 和 subnormal。对 IEEE 风格格式，最大有限数选取最大的**允许有限** bit pattern：`s=0`、$E=2^k-2$、$F=2^m-1$。
...
$$
x_{\max,\mathrm{finite}}=(2-2^{-m})\times2^{E_{\max}}
$$

负侧最小有限值则是：

$$
x_{\min,\mathrm{finite}}=-x_{\max,\mathrm{finite}}
$$

以 FP16 为例，$k=5$、$m=10$、$bias=15$。`11111` 留给 `Inf` / NaN，所以最大有限编码是 `0 11110 1111111111`：

$$
(2-2^{-10})\times2^{30-15}=65504
$$

在实际数轴上，精度也不是一个处处相同的绝对间隔：正常数处于实际指数 $q$ 的 binade 内时，相邻数间隔为 $2^{q-m}$；进入 subnormal 区后，间隔固定为 $2^{1-bias-m}$。这正是浮点数“相对精度大致恒定、绝对精度随数值大小变化”的来源。

### 特殊值策略会改变极值：不能把 IEEE 公式硬套到低比特格式

设计或阅读一种特殊浮点格式时，先检查它的**特殊编码策略**，再计算极值；仅知道 `E4M3` 或 `E5M2` 不够。

| 策略 | 全 1 指数的用途 | 最大有限编码 | 结果与代价 |
| --- | --- | --- | --- |
| IEEE 风格，如 FP16 / FP32 / FP8 E5M2 | `F=0` 是 `Inf`，`F≠0` 是 NaN | $E=2^k-2, F=2^m-1$ | 有无穷与 NaN，最高一个指数档不能表示有限数。 |
| 有限数优先，如 NVIDIA 常见 FP8 E4M3FN | 大部分编码仍放有限数，通常只留少量 NaN 编码，不提供 `Inf` | 必须按规范枚举最后一个合法编码 | 多一个高指数档或更多有限点，但 overflow 不能自然表示为 `Inf`。 |
| 饱和或自定义格式，如部分 FP4 / FP6 路径 | NaN、Inf、subnormal 可能被删除、折叠或由转换规则单独处理 | 由格式规范和转换 API 共同决定 | 值域与编码利用率更高，但异常值语义不再是 IEEE。 |

FP8 E4M3FN 是常见反例：它通常没有 `Inf`，`E=1111_2` 仍可表示有限数，但最高 fraction 编码留作 NaN。因此最大有限编码是 `0 1111 110`，而不是 fraction 全 1：

$$
(1+6\times2^{-3})\times2^{15-7}=1.75\times2^8=448
$$

这也解释了为何它的最大值是常见的 `448`，而不是把 IEEE 公式直接代入 `E4M3` 得出的值。

### 设计或核对一种格式的极值

面对新格式时，按下面顺序推导，能避免把 normal、subnormal 和特殊值混在一起：

1. 写出 `s1 e<k> m<m>`、`bias`，以及每一种保留 bit pattern；不要默认全 1 指数一定是 `Inf` / NaN。
2. 找到最大的**正有限**编码 $(E_{\mathrm{fin,max}}, F_{\mathrm{fin,max}})$，代入 $\left(1+F_{\mathrm{fin,max}}2^{-m}\right)2^{E_{\mathrm{fin,max}}-bias}$。若最高指数不是 normal，必须使用该格式的专门规则。
3. 用 `E=1, F=0` 求最小正 normal；若格式支持 subnormal，再用 `E=0, F=1` 求最小正 subnormal。不要将正零误报为最小正数。
4. 明确 overflow、无效运算和格式转换的输出是 `Inf`、NaN、饱和到最大有限值，还是实现定义值；在 GPU 上这往往取决于具体 CUDA 类型、转换 intrinsic 与编译选项。

下面各格式的小节均以其实际编码和 CUDA 文档为准。尤其是 FP8、FP6、FP4：名称中的 `E` / `M` 只描述字段宽度，**不承诺 IEEE 的特殊值语义**。
