...
Roofline 不是“GPU 有两条神秘曲线”的经验图。它只做一件事：把一个 Kernel 的**计算速度上限**和**数据搬运速度上限**放到同一坐标系里，回答“在它当前的计算量与访存量之比下，最快可能跑到哪里”。

先约定分析对象是一个 Kernel（或一个融合后的算子），并且统一统计口径：

- $F$：这段工作实际完成的**有用浮点运算数**，单位 FLOP。例如一次 FMA 通常按 $2$ FLOP 计。
- $Q$：为了完成这段工作，在某一存储层实际传输的字节数，单位 Byte。研究 HBM/显存时用 DRAM 字节；研究 L2 时就用 L2 字节。不要把“算法最少要读写的字节”和 profiler 统计的 DRAM 字节混在一起。
- $T$：该 Kernel 的稳定实测时间，单位 s。

一个实测点的坐标是：

$$
x = AI = F / Q \quad (\text{FLOP/Byte})
$$

$$
y = P_{measured} = F / T \quad (\text{FLOP/s})
$$

**横坐标 $x$ 不表示“做了多少计算”，而是每搬运 1 Byte 数据平均做了多少 FLOP。** 它描述工作本身对数据复用的需求；越靠右，同一份从 DRAM 搬来的数据支撑的有用计算越多。**纵坐标 $y$ 才是实际跑出的计算速度。** 同一个算法若执行时间减半，点会在同一横坐标向上移动一倍。

### $P_{peak}$ 与 $B_{peak}$：两种完全不同的“每秒能力”

Roofline 需要两个硬件上限，但它们的单位和含义不同：

| 符号 | 单位 | 它在问什么 | 取值时必须匹配什么 |
| --- | --- | --- | --- |
| $P_{peak}$ | FLOP/s，例如 TFLOP/s | GPU 一秒最多能完成多少浮点运算？ | dtype 和计算路径：FP32 CUDA Core、FP16/BF16/TF32/FP8 Tensor Core 不是同一条计算屋顶。 |
| $B_{peak}$ | Byte/s，例如 TB/s | 某一内存层一秒最多能传输多少字节？ | 存储层和访问模式：DRAM、L2、shared memory 各有自己的带宽；优先使用可持续带宽而非包装盒上的理论峰值。 |

$P_{peak}$ 不是“所有指令混在一起的总算力”。若 Kernel 实际没有使用 Tensor Core，就不能拿 FP16 Tensor Core 的数百 TFLOP/s 当它的 $P_{peak}$。同样，$B_{peak}$ 也不是“GPU 一定能跑到的带宽”；随机访问、读写比例、访问未合并、并发请求太少都会让可持续带宽低于规格值。

### 两条线是怎样推出来的

先假设一个便于手算的目标 GPU、目标路径：

$$
P_{peak} = 60\ \text{TFLOP/s}, \qquad B_{peak} = 1.5\ \text{TB/s}
$$

这不是某张具体显卡的规格，只是用来读图的数字。下面两条线都由这两个数推出来。

![Roofline 的两条线、屋脊点和坐标轴含义](/blog-assets/gpu-programming/performance-modeling/roofline-anatomy.svg)

#### 蓝色斜线：带宽屋顶

假设 Kernel 的算术强度是 $AI=2\ \text{FLOP/B}$。这表示每从 DRAM 获得 $1$ Byte，最多只能做 $2$ FLOP；即使显存带宽已经完全跑满，每秒也只能供给：

$$
P_{memory} = AI × B_{peak}
= 2\ \text{FLOP/B} × 1.5\ \text{TB/s}
= 3\ \text{TFLOP/s}
$$

把 $AI$ 换成任意横坐标值，都会得到同一条线：

$$
y = P_{memory}(AI) = AI × B_{peak}
$$

它是一条斜线，不是因为带宽随着 $AI$ 增长，而是因为**同样的每秒字节供给，在每 Byte 能完成更多 FLOP 时，能支撑的 FLOP/s 也更多**。在常见的双对数坐标中，$AI$ 增加 $10$ 倍，这个上限也增加 $10$ 倍，所以看起来是一条斜率为 $1$ 的直线。

#### 绿色水平线：计算屋顶

不管每 Byte 做多少 FLOP，GPU 的目标计算管线每秒最多只能完成 $P_{peak}$ 个 FLOP：

$$
y = P_{compute}(AI) = P_{peak} = 60\ \text{TFLOP/s}
$$

因此它是水平线。向右提高数据复用可以减轻带宽压力，但无法让同一条 FP32 或 Tensor Core 计算管线超过自身每秒的发射能力。

对每个横坐标，真正可达到的上界只能取更低的那条线：

$$
P_{roof}(AI) = \min(P_{peak}, AI × B_{peak})
$$

从时间角度看，这是同一件事：计算至少要花 $F/P_{peak}$，搬运至少要花 $Q/B_{peak}$，理想下界取更慢的一项。
...
两条线相等的位置是 ridge point（屋脊点）：

$$
AI_{ridge} = P_{peak} / B_{peak} = 60 / 1.5 = 40\ \text{FLOP/B}
$$

它的意思很具体：**每从 DRAM 搬来 1 Byte，至少得做约 $40$ FLOP，显存供给速度才有机会跟上这条 60 TFLOP/s 计算管线。**

| 图上的位置 | 两条线谁更低 | 理想世界中先遇到的上限 | 读图结论 |
| --- | --- | --- | --- |
| 屋脊左侧：$AI < 40$ | 蓝色带宽线 | 数据搬运速度 | 这是**带宽受限候选区**。即使计算单元再快，也没有足够的 Byte/s 供它完成更多 FLOP/s。 |
| 屋脊上：$AI = 40$ | 两条线相等 | 计算和带宽同时刚好够用 | 这是平衡点；小幅改变流量、计算路径或实现细节都可能改变瓶颈。 |
| 屋脊右侧：$AI > 40$ | 绿色计算线 | 计算速度 | 这是**计算受限候选区**。DRAM 的理论供给已足够，进一步提高 AI 不会抬高这条计算屋顶。 |

“候选”二字非常重要。左侧并不等于当前已经跑满带宽，右侧也不等于当前已经跑满算力；它们只说：**假如实现把其他问题都解决了，最后最先撞上哪一种硬件上限。**

### 把一个实际 Kernel 放成图上的点

仍用上面的硬件参数。某个 Kernel 统计到：

$$
F = 120\ \text{GFLOP}, \qquad Q = 60\ \text{GB}, \qquad T = 50\ \text{ms}
$$

它在图上的点为：

$$
x = F/Q = 120/60 = 2\ \text{FLOP/B}
$$

$$
y = F/T = 120\ \text{GFLOP}/0.05\ \text{s} = 2.4\ \text{TFLOP/s}
$$

所以点是 $(2, 2.4)$。横坐标 $2$ 小于屋脊点 $40$，其带宽屋顶是 $2 × 1.5 = 3$ TFLOP/s；纵坐标 $2.4$ 接近 $3$，Roofline 效率为 $2.4/3=80\%$。这是一个很像带宽受限的点：优先考虑减少 DRAM 字节、融合相邻算子、提高复用、改善访问合并。

反过来，若另一个 Kernel 的点为 $(100,48)$，其带宽线给出 $100×1.5=150$ TFLOP/s，但计算线只有 $60$ TFLOP/s，因此屋顶为 $60$ TFLOP/s。它已经达到 $48/60=80\%$，更像计算受限：检查是否采用了预期的 Tensor Core / FMA 路径、MMA tile 与流水是否高效、epilogue 是否过重，或是否能在精度允许时减少计算。

### 点离屋顶的距离：用来区分“资源上限”与“没有喂饱”

同一横坐标下，理想屋顶与实测点的纵向差距可以量化为：

$$
E_{roof} = P_{measured} / P_{roof}(AI)
$$

![Roofline 上的四类典型点，以及从点的位置推导优化方向](/blog-assets/gpu-programming/performance-modeling/roofline-points-and-actions.svg)

| 点的位置 | 能否直接下结论 | 常见含义 | 接下来验证什么 |
| --- | --- | --- | --- |
| 左侧且贴近蓝线 | 基本可以判为带宽受限 | 可用带宽已接近可持续上限。 | NCU 中 DRAM 吞吐、读写事务、合并访问；再考虑减少 $Q$。 |
| 右侧且贴近绿线 | 基本可以判为匹配计算路径受限 | 计算管线接近持续峰值。 | Tensor / FMA 利用、指令混合、epilogue；再考虑减少 $F$ 或切换正确的更快路径。 |
| 左侧但远低于蓝线 | **不能**叫带宽受限 | 理论上带宽会限制它，但实际连带宽都没有发满，常见原因是内存延迟、依赖链、CTA 太少。 | Warp stall、可运行 warp、memory-level parallelism、访问是否分散或串行。 |
| 右侧但远低于绿线 | **不能**叫计算受限 | 理论上计算会限制它，但实际计算管线未被喂满。 | 是否走错指令路径、shared memory / 寄存器供给、barrier、分支、occupancy、尾波。 |
| 任意横坐标都很低 | 先排除利用不足 | 小 grid、tail wave、CPU launch gap、stream 串行都能让点很低。 | NSYS 看 GPU 空洞；NCU 看活跃 SM、waves、occupancy 与 scheduler。 |

因此，图中的“空白”并不自动属于带宽或算力。一个 $AI=2$ 的点若只有 $0.1$ TFLOP/s，距离 3 TFLOP/s 的蓝线还很远，真正的问题可能是每个 warp 都在等一条相关 load 返回；此时盲目压缩字节数，往往不如增加独立请求、软件流水或并行工作有效。

### 优化会怎样移动点、怎样改变线

| 改动 | 点或线怎样变化 | 什么时候值得做 |
| --- | --- | --- |
| 减少实际 DRAM 字节 $Q$，有用 FLOP $F$ 不变 | $AI=F/Q$ 变大，点向右；若原本贴蓝线，性能会沿蓝线向上。 | 带宽受限点：融合、消除中间读写、tile 复用。 |
| 改善 coalescing、缓存命中或并发访存 | 横坐标和理论线近似不变，实测点向上靠近蓝线。 | 左侧但离蓝线远：先消除访问低效和延迟。 |
| 减少无用 FLOP | 点可能向左，纵轴 FLOP/s 甚至未必上升，但总时间可下降。 | 任何时候都可能有价值；不要把“向右”当作唯一优化目标。 |
| 改用匹配且允许的更快计算路径 | 对应的绿色 $P_{peak}$ 线抬高，屋脊点也会右移。 | 算法精度与实现确实能使用 FP16/BF16/TF32/FP8 Tensor Core 时。 |
| 增大 grid、增加 ILP、消除 barrier 或 launch gap | 两条理论线不变，实测点从下方向上。 | 延迟受限或利用不足时。 |

真实 GPU 还可以有多条屋顶：FP32、TF32、FP16、FP8 Tensor Core 各有计算屋顶；DRAM、L2、L1/shared memory 也各有带宽屋顶。NCU 的 hierarchical roofline 就是在把“数据卡在哪一层”继续细分。但第一次分析时，先用**实际执行的计算路径 + DRAM 实测字节**画一条计算线和一条 DRAM 线，最容易避免口径混乱。
